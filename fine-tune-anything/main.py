...
if __name__ == "__main__":
    ...
    def preprocess(example):
        ...
        last_user = [m for m in example["messages"] if m["role"] == "user"][-1][
            "content"
        ]
        last_assistant = [m for m in example["messages"] if m["role"] == "assistant"][
            -1
        ]["content"]
        ...
        labels = [-100] * len(tokenizer(input_text)["input_ids"]) + tokenized[
            "input_ids"
